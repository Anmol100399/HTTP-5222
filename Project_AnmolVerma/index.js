...
app.get('/events/details/:eventId', async (req, res) => {
    ...
    const weatherDetails = await weather.getWeather(latitude,longitude);
...
app.get('/events/search', async (req, res) => {
    ...
    if (req.query.q) {
        ...
        res.render('event-search', { events: filteredItems, page: page, totalPages: Math.round(eventIds.length / 20), searchQuery: req.query.q });
    } else {
        res.render('event-search', { events: events, page: page, totalPages: Math.round(eventIds.length / 20), searchQuery: '' });
    }
});
